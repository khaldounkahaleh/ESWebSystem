...
function updateDebugInfo() {
  ...
  agentTypes.forEach((type) => {
    ...
    agentInfo += `
      <div class="agentType ${type.replace(" ", "-")}-text">
        <p><strong>${
...
function saveWeights() {
  ...
  a.href = url;
  a.download = `neural-life-14-weights-${getTimestamp()}.json`;
  a.click();
...
class Agent {
  constructor(x, y, type, nn = null) {
    ...
    this.state = "moving";
    this.speed =
      type === "predator" ? 0.5 : type === "prey" ? 0.4 : type === "super predator" ? 0.6 : 0.3;
    this.nn = nn ? nn : new NeuralNetwork(19, 8, 2); // Updated inputNodes to 19
    this.age = 0;
    this.beatEffect = 0;
    ...
    this.communication = 0; // Energy level communicated by nearby agents
    this.emotionalState = 0; // Emotional state (e.g., fear level)
  ...
  update() {
    ...
    // Record memory
    this.memory.positions.push({ x: this.x, y: this.y });
    this.memory.energies.push(this.energy);

    if (this.memory.positions.length > 10) {
      this.memory.positions.shift();
      this.memory.energies.shift();
    }
    ...
    this.y = Math.max(
      0,
      Math.min(rows - 1, this.y + Math.sign(reproductionValue - 0.5 + randomMoveY))
    );
    ...
    obstacles.forEach((obstacle) => {
      if (this.x === obstacle.x && this.y === obstacle.y) {
        this.energy -= 10; // Lose energy when colliding with an obstacle
    ...
    if (collidedWithObstacle) {
      // Additional logic for collision, if any
    }
    ...
    if (resources[key]) {
      this.energy += resources[key].energy; // Gain energy from resources
      delete resources[key];
      this.updated = true;
    }
  ...
  getInputs() {
    ...
    let nearestScavengerDistance = Infinity;
    let nearestExplorerDistance = Infinity;
    let nearestBuilderDistance = Infinity;
    let nearestHealerDistance = Infinity;
    let nearestObstacleDistance = Infinity;

    let nearestPredatorAngle = 0;
    let nearestPreyAngle = 0;
    let nearestResourceAngle = 0;
    let nearestScavengerAngle = 0;
    let nearestExplorerAngle = 0;
    let nearestBuilderAngle = 0;
    let nearestHealerAngle = 0;
    let nearestObstacleAngle = 0;

    let communicatedEnergy = 0;
    ...
    agents.forEach((agent) => {
      if (agent !== this) {
        ...
        const angle = Math.atan2(dy, dx) / (2 * Math.PI); // Normalize angle to [-0.5, 0.5]

        if (agent.type === "predator" && distance < nearestPredatorDistance) {
          nearestPredatorDistance = distance;
          nearestPredatorAngle = angle;
          this.emotionalState = 1; // Set fear level
        } else if (agent.type === "prey" && distance < nearestPreyDistance) {
          nearestPreyDistance = distance;
          nearestPreyAngle = angle;
        } else if (agent.type === "scavenger" && distance < nearestScavengerDistance) {
          nearestScavengerDistance = distance;
          nearestScavengerAngle = angle;
        } else if (agent.type === "explorer" && distance < nearestExplorerDistance) {
          nearestExplorerDistance = distance;
          nearestExplorerAngle = angle;
        } else if (agent.type === "builder" && distance < nearestBuilderDistance) {
          nearestBuilderDistance = distance;
          nearestBuilderAngle = angle;
        } else if (agent.type === "healer" && distance < nearestHealerDistance) {
          nearestHealerDistance = distance;
          nearestHealerAngle = angle;
        }

        if (distance < 5) { // Arbitrary communication range
          communicatedEnergy += agent.energy;
        }
      }
    });
    ...
    const lastPosition = this.memory.positions.length ? this.memory.positions[this.memory.positions.length - 1] : { x: 0, y: 0 };
    const lastEnergy = this.memory.energies.length ? this.memory.energies[this.memory.energies.length - 1] : 0;
    ...
    return [
      ...
      nearestScavengerDistance / Math.max(cols, rows),
      nearestExplorerDistance / Math.max(cols, rows),
      nearestBuilderDistance / Math.max(cols, rows),
      nearestHealerDistance / Math.max(cols, rows),
      nearestObstacleDistance / Math.max(cols, rows), 
      distanceToLeftWall,
      distanceToRightWall,
      distanceToTopWall,
      distanceToBottomWall,
      audioFeatures.average / 256,
      audioFeatures.isBeat ? 1 : 0,
      nearestPredatorAngle,
      nearestPreyAngle,
      nearestResourceAngle,
      nearestScavengerAngle,
      nearestExplorerAngle,
      nearestBuilderAngle,
      nearestHealerAngle,
  ...
  render() {
    ...
    if (this.updated) {
      ...
      if (index >= 0 && index < container.children.length) {
        ...
        updateElementContent(element, char, color);
      } else {
        console.error(
...
class Obstacle {
  ...
  validateCoordinates() {
    if (
      ...
      console.error(
        `Obstacle has invalid coordinates: resetting position`
      );
      this.x = Math.floor(Math.random() * cols);
      this.y = Math.floor(Math.random() * rows);
    }
  ...
  update() {
    ...
    const period = 100; // Period of the sine wave

    this.x += Math.round(this.dx * Math.sin((seconds / period) * 2 * Math.PI));
    this.y += Math.round(this.dy * Math.sin((seconds / period) * 2 * Math.PI));
    ...
    this.colorPhase += 0.05; // Increment the phase for the pulsating effect
    this.charPhase = (this.charPhase + 1) % this.chars.length; // Cycle through characters

    this.validateCoordinates();
  }

  getPulsatingColor() {
    const r = Math.floor((Math.sin(this.colorPhase) + 1) * 127);
    const g = Math.floor((Math.sin(this.colorPhase + 2) + 1) * 127);
    const b = Math.floor((Math.sin(this.colorPhase + 4) + 1) * 127);
    return `rgb(${r},${g},${b})`;
  }
  ...
  render() {
    ...
    if (index >= 0 && index < container.children.length) {
      const element = container.children[index];
      const char = this.chars[this.charPhase];
      const color = this.getPulsatingColor();
      updateElementContent(element, char, color);
    } else {
      console.error(
        `Invalid obstacle index during rendering: ${index}, x: ${this.x}, y: ${this.y}, cols: ${cols}, rows: ${rows}`
      );
      this.x = Math.floor(Math.random() * cols);
      this.y = Math.floor(Math.random() * rows);
    }
...
class FractalEvent {
  constructor(x, y) {
    ...
    this.active = true;
    this.iteration = 0;
    this.maxZoom = 10; // Cap the maximum zoom level
  ...
  update() {
    if (this.iteration < this.maxIter && this.zoom < this.maxZoom) {
      this.iteration++;
      this.zoom *= 1.15; 
      // this.maxIter = Math.max(10, 50 - Math.floor(this.zoom)); // Reduce iterations as zoom increases
    } else {
      this.active = false;
    }
  }
  ...
  render() {
    for (let x = 0; x < cols; x++) {
      for (let y = 0; y < rows; y++) {
        ...
        const colorIndex = Math.floor((n / this.maxIter) * (this.fractalColors.length - 1));
        if (element.textContent !== chars[charIndex] || element.style.color !== this.fractalColors[colorIndex]) {
          element.textContent = chars[charIndex];
          element.style.color = this.fractalColors[colorIndex];
        }
      }
    }
  }
...
function initializeObstacles() {
  obstacles = [];
  for (let i = 0; i < 256; i++) { // Adjust the number of obstacles as needed
    const x = Math.floor(cols / 2 + (Math.cos(i) * (cols / 2)));
    const y = Math.floor(rows / 2 + (Math.sin(i) * (rows / 18)));
    const dx = Math.cos(i);
    const dy = Math.sin(i);
    obstacles.push(new Obstacle(x, y, dx, dy));
  }
}

function updateElementContent(element, char, color) {
...
function reinitializeObstacles() {
  obstacles.forEach(obstacle => {
    obstacle.validateCoordinates();
  });
}
...
class Boost {
  ...
  validateCoordinates() {
    if (
      ...
      console.error(`Boost has invalid coordinates: resetting position`);
      this.x = Math.floor(Math.random() * cols);
      this.y = Math.floor(Math.random() * rows);
    }
  }
  ...
  update() {
    const period = 150; // Period of the sine wave
    this.x += Math.round(this.dx * Math.sin((frameCount / period) * 4 * Math.PI));
    this.y += Math.round(this.dy * Math.sin((frameCount / period) * 4 * Math.PI));
    ...
    this.charPhase = (this.charPhase + 1) % this.chars.length;
    this.validateCoordinates();
  }
  ...
  render() {
    ...
    if (index >= 0 && index < container.children.length) {
      ...
      console.error(
        `Invalid boost index during rendering: ${index}, x: ${this.x}, y: ${this.y}, cols: ${cols}, rows: ${rows}`
      );
      this.x = Math.floor(Math.random() * cols);
      this.y = Math.floor(Math.random() * rows);
    }
...
function initializeBoosts() {
  boosts = [];
  for (let i = 0; i < 64; i++) { // Adjust the number of boosts as needed
    const x = Math.floor(cols / 2 + (Math.cos(i) * (cols / 8)));
    const y = Math.floor(rows / 2 + (Math.sin(i) * (rows / 8)));
    const dx = Math.cos(i);
    const dy = Math.sin(i);
    boosts.push(new Boost(x, y, dx, dy));
  }
}

function reinitializeBoosts() {
  boosts.forEach(boost => {
    boost.validateCoordinates();
  });
}
...
function animateGrid(timestamp) {
  ...
  if (fractalEvent) {
    ...
    if (agents.length < 10) {
      if (agents.every(agent => agent.type === "normal")) {
        const x = Math.floor(Math.random() * cols);
        const y = Math.floor(Math.random() * rows);
...
// Add event listener for Enter key to trigger a fractal event
...
window.onload = () => {
  ...
  document
    ...
    var element = document.getElementById("GoHome");
    element.setAttribute('onclick', 'window.location.href=\'../index.html\'');
